...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] InstanceManagerNetworkObject = null;
		public GameObject[] LootingGameNetworkObject = null;
		...
		public GameObject[] TeleporterNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is InstanceManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (InstanceManagerNetworkObject.Length > 0 && InstanceManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(InstanceManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<InstanceManagerBehavior>();
						}
					}
			...
			else if (obj is LootingGameNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (LootingGameNetworkObject.Length > 0 && LootingGameNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(LootingGameNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<LootingGameBehavior>();
						}
					}
			...
			else if (obj is TeleporterNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TeleporterNetworkObject.Length > 0 && TeleporterNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TeleporterNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<TeleporterBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateInstanceManager instead, its shorter and easier to type out ;)")]
		public InstanceManagerBehavior InstantiateInstanceManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(InstanceManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<InstanceManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<InstanceManagerBehavior>().networkObject = (InstanceManagerNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateLootingGame instead, its shorter and easier to type out ;)")]
		public LootingGameBehavior InstantiateLootingGameNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(LootingGameNetworkObject[index]);
			var netBehavior = go.GetComponent<LootingGameBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<LootingGameBehavior>().networkObject = (LootingGameNetworkObject)obj;
		...
		[Obsolete("Use InstantiateTeleporter instead, its shorter and easier to type out ;)")]
		public TeleporterBehavior InstantiateTeleporterNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TeleporterNetworkObject[index]);
			var netBehavior = go.GetComponent<TeleporterBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<TeleporterBehavior>().networkObject = (TeleporterNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of InstanceManager
		/// </summary>
		/// <returns>
		/// A local instance of InstanceManagerBehavior
		/// </returns>
		/// <param name="index">The index of the InstanceManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public InstanceManagerBehavior InstantiateInstanceManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(InstanceManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<InstanceManagerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of LootingGame
		/// </summary>
		/// <returns>
		/// A local instance of LootingGameBehavior
		/// </returns>
		/// <param name="index">The index of the LootingGame prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public LootingGameBehavior InstantiateLootingGame(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(LootingGameNetworkObject[index]);
			var netBehavior = go.GetComponent<LootingGameBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Teleporter
		/// </summary>
		/// <returns>
		/// A local instance of TeleporterBehavior
		/// </returns>
		/// <param name="index">The index of the Teleporter prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public TeleporterBehavior InstantiateTeleporter(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TeleporterNetworkObject[index]);
			var netBehavior = go.GetComponent<TeleporterBehavior>();
